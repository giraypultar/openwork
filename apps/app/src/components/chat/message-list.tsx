...
import { groupMessages, isMessageGroup, getLastTextPart, getAssistantRenderGroups, type UIMessageWithIndex, getMessagesText } from "./utils"
...
export const AssistantMessage = React.memo(
  ({ message, isLastMessage, isStreaming, isLastStep }: AssistantMessageProps) => {
    const { showThinking, onRevertToUserMessage, onForkAtMessage } = useMessageList()
...
const getTextMessages = (messages: UIMessageWithIndex[]) =>
  messages.flatMap((item) => {
    const parts = item.message.parts.filter((part) => part.type === "text");
...
function MessageGroup({
  ...
  const itemsWithText = getTextMessages(items)
  ...
  return (
    <LayoutGroup>
      ...
      <AnimatePresence initial={false}>
        {!open ? itemsWithText.map(({ index, message }) => (
          <motion.div
            key={message.id}
      ...
      {lastTextMessage && !isStreaming && (
        <div className="mx-auto flex w-full max-w-3xl flex-wrap items-center gap-2 px-2 opacity-0 transition-opacity duration-150 group-hover/message-group:opacity-100 md:px-8">
          <MessageActions className="flex gap-0">
            <CopyMessageButton messages={itemsWithText.map((item) => item.message)} />
            <MessageAction tooltip="Branch in new chat">
              <Button
                variant="ghost"
                size="icon"
      ...
      {!lastTextMessage && !isStreaming ? <EmptyMessage /> : null}
      </div>
    </LayoutGroup>
  )
}
...
export function MessageList({ messages, status }: MessageListProps) {
  ...
  return (
    <div className={cn("flex flex-col gap-2 @container/message-list")}>
      ...
      {/* {JSON.stringify(messages.map(m => m.parts.map(p => { return { type: p.type, role: m.role } } )))} */}
    </div>
  )
}
